	...
	implements GLEventListener
{
	...
	//since it will disappear in 20 times.
	...
	//this will be set to current-diff each time when it hit the wall.
		...
		switch(container)
		{
			...
			case 1:
					...
					if(currentx+r>=xright)
		        {
		        	...
		        	bouncecount++;
		        	//update teh transparency when it hit the wall
		        ...
		        else if(currentx-r<=xleft)
		        {
		        	...
		        	startx=currentx;
		        	starty=currenty;
		        	//give it a down impulsespeed
		        	speed=speed*impulseDown;
		        	bouncecount++;
		        	colormagnitude=colormagnitude-diff;
		        	//update the current color so it will have the relection side color
		   ...
		   case 2:
			    ...
			    break;
            //case for conatiner 3
		    case 3:
		    	index1=checkIfReturnAnyIntersection(pointlist,startpoint,direction);
		    	//when the index of the side is an odd number, then impulse down when it is 
		    	//an even numbver then pulse it up.
        		if(index1%2==0)
        		{
                  speed=speed*impulseUp;
        		}

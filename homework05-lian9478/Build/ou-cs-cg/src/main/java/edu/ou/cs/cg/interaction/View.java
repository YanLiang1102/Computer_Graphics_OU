	...
	implements GLEventListener
{
    ...
    public boolean counterclockwise=true;
  ...
  //       gl.glPushMatrix(); // save the current matrix
	...
	{
		//count=1;
		//just need to set the centery to be 0 for each node and the space will be a litlle bit over its withd
		 int size=nodelist.size();
		 nodelist.get(0).centerx=-1.0f+nodelist.get(0).width;
		 //double previous=-1.0f+width;
		 nodelist.get(0).centery=0.0f;
		 for(int i=1;i<size;i++)
		 {
		 	 nodelist.get(i).centery=0.0f;
		 	 nodelist.get(i).centerx=nodelist.get(i-1).centerx+2* nodelist.get(i).width;
